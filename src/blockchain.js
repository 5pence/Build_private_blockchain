    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if ( (currentTime - getTheTimeFromMessage) < 300) {
                ...
                reject(Error("Over 5 minutes has elapsed - rejected!"))
            }
        });
    }
    ...
    getBlockByHash(hash) {
        ...
        return new Promise((resolve, reject) => {
           ...
           if (typeof block !== 'undefined') {
               ...
               reject(Error('There is no block with this hash, hash invalid!'))
           }
        });
    }
    ...
    getStarsByWalletAddress (address) {
        ...
        return new Promise((resolve, reject) => {
            ...
            self.chain.forEach( async (blockBodyDecodedData) => {
                // get the blocks data
                let data = await blockBodyDecodedData.getBData();
                if (data) {
                    // check block owner and address align
            ...
            self.chain.forEach((block, index) => {
                ...
                if (block.height > 0) {
                    ...
                    if (block.previousBlockHash !== previousBlock.hash) {
                        // update error log of mismatches
                        errorLog.push(`Block ${index} previous hash mismatch. previousBlockHash: 
                        ${block.previousBlockHash} vs previousBlock.hash: ${previousBlock.hash}`);
                    }
                }
                // push to validateBlocks array
                validatedBlocks.push(block.validate());
            })
            // collect the validated promises
            Promise.all(validatedBlocks).then(validatedBlocks=> {
                // loop through the blocks with an index
                validatedBlocks.forEach((valid, index) => {
                    // if they are invalid then push an error to the error log
                    if (!valid) {
                        errorLog.push(`Block ${index} hash ${self.chain[index].hash} is invalid!`)
                    }
                });
                resolve(errorLog);
            });
